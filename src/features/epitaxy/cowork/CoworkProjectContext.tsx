...
export function CoworkProjectWarningDialog({
  ...
  return (
    <OfficialModal isOpen={Boolean(pending)} onClose={onCancel} title={title} width="w-[520px]">
      <div data-official-source={coworkProjectWarningOfficialSource}>
        ...
        <div className="mt-5 flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <OfficialButton onClick={onCancel} size="small" variant="contained">Cancel</OfficialButton>
            <OfficialButton onClick={() => pending && onConfirm(pending.project, dontShowAgain)} size="small" variant="primary">{confirmLabel}</OfficialButton>
          </div>
        </div>
      </div>
    </OfficialModal>
