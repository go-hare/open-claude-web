...
}: CreateCoworkAddMenuItemsOptions): OfficialDropdownItem[] {
...
function createCoworkFileItems({
  ...
  openProjectCreate,
  projects,
}: Pick<CreateCoworkAddMenuItemsOptions, "includeAddFolder" | "onAddFiles" | "onAddFolder" | "onSelectProject" | "projects"> & { openConnectors: () => void; openProjectCreate: () => void }): OfficialDropdownItem[] {
  return [
    {
      icon: "FileAdd",
...
function createProjectSubmenuItem(projects: CoworkAddMenuProject[], onSelectProject: ((project: CoworkAddMenuProject) => void) | undefined, openProjectCreate: () => void): OfficialDropdownItem[] {
...
function createProjectMenuItems(projects: CoworkAddMenuProject[], onSelectProject: ((project: CoworkAddMenuProject) => void) | undefined, openProjectCreate: () => void): OfficialDropdownItem[] {
  const projectItems = projects.slice(0, 8).map((project): OfficialDropdownItem => ({
  ...
  return [
    ...projectItems,
    ...(projectItems.length ? [{ label: "projects-separator", type: "separator" } satisfies OfficialDropdownItem] : []),
    { icon: "Add", label: "Start a new project", onSelect: openProjectCreate },
  ];
}

function createDriveMenuItem(): OfficialDropdownItem {
  return {
    icon: "Cloud",
    items: [
...
function createCoworkToolItems({ openConnectors, openPlugins, openSkills }: CoworkAddMenuRoutes): OfficialDropdownItem[] {
  return [
    {
      icon: "Plugin",
      items: [
...
function createCoworkModeItems({
  ...
  openStyles,
  webSearchEnabled,
}: Pick<CreateCoworkAddMenuItemsOptions, "onToggleWebSearch" | "webSearchEnabled"> & { openStyles: () => void }): OfficialDropdownItem[] {
  return [
    {
