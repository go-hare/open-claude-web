...
function OfficialSkillChipNodeView({ node, selected }: NodeViewProps) {
...
export const OfficialSkillChip = Node.create({
  name: "skillChip",
  group: "inline",
  inline: true,
  atom: true,
  ...
  addNodeView() {
    return ReactNodeViewRenderer(OfficialSkillChipNodeView);
  },

  addKeyboardShortcuts() {
    return {
      Backspace: () => selectPreviousOfficialChip(this.editor, this.name),
    };
  },
});

function selectPreviousOfficialChip(editor: CommandProps["editor"], nodeName: string) {
