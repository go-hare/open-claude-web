...
export function CoworkActivitySection({ children, className, contentClassName, headerClassName, headerLeftAction, isExpanded, maxContentHeight, onToggle, title }: CoworkActivitySectionProps) {
  return (
    <div className={`rounded-lg bg-bg-100 border-0.5 border-border-300 shadow-sm overflow-hidden shrink-0 ${className ?? ""}`}>
      <CoworkActivitySectionHeader className={headerClassName} headerLeftAction={headerLeftAction} isExpanded={isExpanded} title={title} onToggle={onToggle} />
...
function CoworkActivitySectionHeader({ className, headerLeftAction, isExpanded, onToggle, title }: { className?: string; headerLeftAction?: ReactNode; isExpanded: boolean; onToggle: () => void; title: ReactNode }) {
