...
export function CoworkScheduledSection({ frame, items, onNavigate, selectedSessionId }: { frame: FrameStore; items: CoworkScheduledSidebarItem[]; onNavigate: (path: string) => void; selectedSessionId: string | null }) {
  ...
  return (
    <section className="group/section flex flex-col gap-px" data-cowork-sidebar-section="scheduled">
      <SidebarSectionHeader collapsed={collapsed} onToggle={() => frame.toggleGroupCollapsed("scheduled")}>{text.scheduledTasks}</SidebarSectionHeader>
...
function CoworkScheduledRow({ item, onNavigate, selected }: { item: CoworkScheduledSidebarItem; onNavigate: (path: string) => void; selected: boolean }) {
  ...
  const rowKey = `scheduled-task:${item.task.id}`;
  return (
    <div className={rowWrapperClass} data-jump-hint-row="" data-row-key={rowKey} data-selected={selected ? "open" : undefined}>
      <button className={rowButtonClass} data-row="" data-roving-item="" data-row-main-button="" data-selected={selected ? "open" : undefined} onClick={() => onNavigate(target)} type="button">
...
/** Official SidebarItem session chrome — jump nav targets [data-row-main-button] under data-kind. */
